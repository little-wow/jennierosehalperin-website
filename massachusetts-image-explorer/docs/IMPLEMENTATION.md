...
## Diagnostics payload
The proxy now returns richer failure fields:
- `requestId`: trace id for one proxy attempt
- `category`: failure class (`dns`, `connectivity`, `tls`, `timeout`, `unknown`)
- `code`: low-level system error code when available
- `detail`: thrown fetch error message
